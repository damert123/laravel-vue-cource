...
export default {
    ...
    methods:{
        ...
        getComments(post){
                axios(route('posts.comments.index', post.id))
                    .then( res => {
        ...
        storeComment(post){
            if(this.parentId){
                this.newComment.parent_id = this.parentId.id
            }

            axios.post(route('posts.comments.store', post.id), this.newComment)
                .then( res => {
                    // this.getComments(post)
                    ...
                    if (!this.parentId){
                        ...
                        this.parentId.replies.push(res.data)
                    }
                    this.unsetParentId()
        ...
        deleteComment(post, comment){
            axios.delete(route('posts.comments.destroy', [post.id, comment.id]))
                .then( res => {
...
<template>
    <div class="max-w-2xl mb-12 mx-auto bg-customBlack rounded-xl shadow-md overflow-hidden">
        <div class="p-6">
            ...
            <div class="flex items-center justify-between space-x-6">
                ...
                <div class="flex items-center space-x-6">
                    ...
                    <button @click="openComment(post)"  class="flex items-center space-x-2 text-white hover:text-blue-600 transition duration-150">
                        <svg  class="w-6 h-6" viewBox="0 0 42 42" fill="none" stroke="currentColor" xmlns="http://www.w3.org/2000/svg">
        ...
        <div v-if="isComment" class="">

            <div v-for="comment in comments" :key="comment.id" class=" py-4 px-6 border-t border-gray-600 text-white">

                <div class="py-4 px-6 border-t border-gray-600 text-white">
                    <!-- Основной комментарий -->
                    <div class="flex justify-between">
                        <p><strong>{{ comment.username }}</strong></p>
                        <p class="text-stone-500 text-sm">{{ comment.created_at }}</p>
                    </div>
                    <p>
                        {{ comment.title }}
                    </p>
                    <a @click.prevent="setParentId(comment)" class="text-stone-400" href="#">Ответить</a>
                    <a  @click.prevent="deleteComment(post, comment)" class="text-red-400 ml-3" href="#">Удалить</a>
                    <div>
                         <a v-if="comment.replies.length" @click.prevent="openReply(comment)" class="text-stone-300 ml-4" href="#">
                             {{ comment.showReplies ? 'Скрыть ответы' : `Показать ${comment.replies.length} ${comment.replies.length === 1 ? 'ответ' : 'ответов'}` }}</a>
                    </div>


                    <!-- Вложенные ответы -->
                    <div v-if="comment.showReplies" v-for="reply in comment.replies" :key="reply.id" class="ml-8 mt-4">
                        <div class="flex justify-between">
                            <p>
                                <strong>{{ reply.username }}</strong>
                                <span class="text-sky-500"> @{{ reply.replyName }}</span>
                            </p>
                            <p class="text-stone-500 text-sm">{{ reply.created_at }}</p>
                        </div>
                        <p>{{ reply.title }}</p>
                        <a @click.prevent="setParentId(comment)" class="text-stone-400" href="#">Ответить</a>
                        <a  @click.prevent="deleteComment(post, reply)" class="text-red-400 ml-3" href="#">Удалить</a>
            ...
            <div class="mb-4 p-2">
                <div class="text-stone-400" v-if="parentId">
                   <p class=" flex border w-fit p-2 border-gray-500 border-b-0 rounded-t-md">Ответить: {{parentId.title}}<svg @click="unsetParentId" class="w-5 ml-2 cursor-pointer" viewBox="0 0 50 50" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <div class="flex items-center">
                    <input v-model="newComment.title" type="text" class="w-full text-white bg-customBlack p-2 border border-gray-500 rounded" placeholder="Написать комментарий...">
                    <button @click="storeComment(post)" class=" px-4 py-2 bg-blue-600 text-white rounded">Отправить</button>
                </div>
